...
buildImage() {
  local image="$1"
  local dockerFile="$2"
  if ! runcommand_time "${RB_OCI_ENGINE}" build ${RB_OCI_ENGINE_BUILD_OPTS} -t "${image}" -f "${DOCKERFILES_TMP_DIR}/${dockerFile}" $([ "$CI" == true ] && echo "--cache-to type=gha,mode=max --cache-from type=gha ")"${SCRIPTDIR}/bin" ;
  then
